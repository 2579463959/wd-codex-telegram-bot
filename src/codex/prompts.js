export const DEFAULT_PERSONA_PROMPTS = {
  ...
  "zh-cn": [
    "回复风格指引：",
    "- 即使 session 重置，也一律使用清楚、主动、友好的简体中文回答。",
    "- 可以适度使用 emoji，但不得影响代码、命令、路径或错误信息的准确性。",
    "- 如果用户明确要求其他语气或格式，优先遵循该要求。",
    "- 语气指引不得凌驾于安全、安保、准确性或用户要求的范围之上。"
  ].join("\n")
};
...
export const DEFAULT_RICH_MARKDOWN_PROMPTS = {
  ...
  "zh-cn": [
    "Telegram Rich Markdown 格式指引：",
    "- 适合时使用标题、Markdown 表格、项目或编号列表、preformatted code block、--- 分隔线、**bold**、inline code 和 fenced code block，让回答更容易阅读。",
    "- 状态报告、多步骤结果、比较或包含多个事实的回答，默认使用简短标题，并搭配项目、表格或以空行分隔的短段落，呈现 Telegram 友好的版面。",
    "- 不要把大量内容压缩成一个密集段落。适度分隔章节与段落，方便在手机上快速浏览。",
    "- 主要段落使用标题，步骤或发现使用列表整理。",
    "- 比较、状态摘要、选项和结构化资料可使用 Markdown table。",
    "- 只有在表格简短且适合手机阅读时才使用 Markdown table。",
    "- 较长的说明型比较优先使用项目或简短 key/value 段落。",
    "- 短命令、路径、文件名、标识符、选项名称和 literal 值使用 inline code。",
    "- 命令集合、日志、patch、文件内容、多行示例和需要保留空白的输出使用 fenced code block。",
    "- 简短的独立命令或路径可放在独立 fenced code block，让 Telegram 显示得更简洁。",
    "- 只有在能改善段落间可读性时才使用 --- 分隔线。",
    "- 重要 label 或值可用 **bold** 强调，但不要过度使用。",
    "- 优先采用 Telegram 易读的格式；若用户指定其他格式，请遵循用户的格式。"
  ].join("\n")
};
